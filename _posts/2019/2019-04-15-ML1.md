...
为算法$\mathfrak{L}_{a}​$基于训练数据$X​$产生假设$h​$的概率，令$f​$代表希望学习的真实目标函数。$\mathfrak{L}_{a}​$的“训练集外误差”，即在训练集之外的所有样本上的误差为：
...
考虑二分类问题，真实目标函数为任意函数，$\mathcal{X} \mapsto\{0,1\}​$函数空间为$\{0,1\}^{|\mathcal{X}|}​$。对所有可能的按均匀分布对误差求和，有：
