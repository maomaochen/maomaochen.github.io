...
假设样本空间$\mathcal{X}$和假设空间$\mathcal{H}$都是离散的，令$P\left(h|X,\mathfrak{L}_{a}\right)​$为算法基于训练数据产生假设的概率，令代表希望学习的真实目标函数。的“训练集外误差”，即在训练集之外的所有样本上的误差为：
...
考虑二分类问题，真实目标函数为任意函数，函数空间为。对所有可能的按均匀分布对误差求和，有：
...
无论有多好、有多坏，它们的期望性能是一样的，只就是**“没有免费的午餐”定理**（No Free Lunch Theorem,NFL）。
